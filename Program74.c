//Accept N numbers from user and count number of Odd no.s and return the count

//Worst case: Array does not contain any Odd elements
//Best case: All elements of array are Odd

//Time Complexity: 0(n)


#include<stdio.h>  
#include<stdlib.h>  //for malloc and free

int OddCount(int *Arr,int iSize)
{
    int iCnt=0;
    int iOddCount=0;
    
    for(iCnt=0;iCnt<iSize;iCnt++)
    {
        if((Arr[iCnt]%2)!=0)     //or  (Arr[iCnt]%2)==1
        {
            iOddCount++;
        }    
    }   

    printf("\n");
    return iOddCount;
}

int main()
{
    int iCount=0;
    int *ptr=NULL; 
    printf("\nEnter the no. of elements that you want to enter: ");
    scanf("%d",&iCount);

    ptr=(int*)malloc(iCount*sizeof(int));
    printf("\nDynamic memory gets allocated successfully for %d elements",iCount);

    int iCnt=0;

    printf("\nEnter the %d values",iCount);

        //1        //2      //3
    for(iCnt=0;iCnt<iCount;iCnt++)
    {
        printf("\nEnter the element no.%d: ",iCnt+1);
        scanf("%d",&ptr[iCnt]);//4    //  internally considered as   &(*(ptr+iCnt))
    }

    int iRet=OddCount(ptr,iCount);  //Display(100,4)   //passing address and no. of elements to the function
    printf("\nThe Count of Odd numbers is: %d",iRet);


    free(ptr);  //free(100)   //Memory manager of OS performs the task of freeing the memory


    printf("\nDynamic memory gets deallocateed successfully...");
    
    return 0;
}